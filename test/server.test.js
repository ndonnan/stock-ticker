...
describe('Server', function () {
  ...
  it('gets a quote', async function () {
    ...
    expect(res.result.quote).to.equal(199.23)
  })
  ...
  it('returns an error if the symbol is not found', async function () {
    ...
    expect(res.result).to.equal('Unknown symbol: UNKNOWN')
  })
  ...
  it('return an error if there is a general server error', async function () {
    ...
    expect(res.result).to.equal('A general server error has occurred')
  })
})
